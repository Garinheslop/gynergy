...
export const deleteData = async (url, data, token) => {
  ...
  try {
    const res = await axios.delete(`${baseUrl}${url}`, data, { headers: headers, withCredentials: true });
    return res.data;
  } catch (error) {
    return { error };
  }
...
export const uploadImage = async (token, url, imageType, data) => {
  ...
  try {
    const res = await axios.post(`${baseUrl}${url}/${imageType}`, data, { headers: headers, withCredentials: true });
    return res.data;
  } catch (error) {
    return { error };
  }
