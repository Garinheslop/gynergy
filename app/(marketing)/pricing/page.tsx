...
export default function PricingPage() {
  ...
  const handleRedeemCode = async (code: string) => {
    if (!session) {
      ...
      router.push(`/?redirect=/pricing&code=${code}`);
  ...
  // If user has challenge access, show different content
      ...
      <section className="bg-action-600 py-16">
        <div className="mx-auto max-w-4xl px-4 text-center sm:px-6 lg:px-8">
          ...
          <button
            ...
            disabled={checkoutLoading || hasChallengeAccess}
            className={cn(
              "text-action-600 rounded-lg bg-white px-8 py-4 text-lg font-bold",
              "hover:bg-action-50 transition-colors",
              "disabled:cursor-not-allowed disabled:opacity-60"
            )}
          >
            {hasChallengeAccess ? "You Have Access!" : "Start Your 45-Day Journey - $997"}
          </button>
        </div>
      </section>
