        ...
        <div className="mx-auto max-w-4xl px-4 py-8">
          <div className="animate-pulse">
            ...
            <div className="mb-8 flex items-center gap-6">
              <div className="bg-grey-100 h-24 w-24 rounded-full" />
              <div className="space-y-3">
                <div className="bg-grey-100 h-6 w-48 rounded" />
            ...
            <div className="mb-8 grid grid-cols-3 gap-4">
              {[1, 2, 3].map((i) => (
                <div key={i} className="bg-grey-100 h-24 rounded" />
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="bg-bkg-light-secondary min-h-screen">
        <div className="mx-auto max-w-4xl px-4 py-8">
          <div className="border-danger/30 bg-danger/10 rounded border p-6 text-center">
            ...
            <Link
              href="/community"
              className="text-action-600 hover:text-action-700 mt-4 inline-block"
            >
              Back to Community
            </Link>
          </div>
    ...
    <div className="bg-bkg-light-secondary min-h-screen">
      ...
      <div className="mx-auto max-w-4xl px-4 py-8">
        ...
        <div className="mb-8 flex flex-col items-center gap-6 sm:flex-row sm:items-start">
          {/* Avatar */}
          <div className="bg-grey-100 relative h-24 w-24 flex-shrink-0 overflow-hidden rounded-full">
            {member.profileImage ? (
              <Image
          ...
          <div className="flex-1 text-center sm:text-left">
            <h1 className="text-content-dark text-2xl font-bold">
              {member.firstName} {member.lastName}
            </h1>
            {member.cohort && (
              <p className="text-grey-500 mt-1">
                Member of <span className="text-action-600">{member.cohort.name}</span>
              </p>
            )}
            {member.bio && <p className="text-content-dark-secondary mt-2">{member.bio}</p>}
            {member.location && (
            ...
            {!isOwnProfile && (
              <div className="mt-4 flex flex-wrap gap-3">
                <button
                  ...
                  className="from-primary to-primary-500 text-content-dark focus-visible:ring-action min-h-[44px] rounded bg-gradient-to-r px-6 py-2 font-medium transition-opacity hover:opacity-90 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none disabled:opacity-50"
                >
                  {isEncouraging ? "Sending..." : "Send Encouragement"}
                </button>
                <button
                  onClick={() => router.push(`/community/messages?userId=${member.id}`)}
                  className="border-action-600 text-action-600 hover:bg-action-50 focus-visible:ring-action min-h-[44px] rounded border px-6 py-2 font-medium transition-colors focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none"
                >
                  Send Message
                </button>
              </div>
        ...
        <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-4">
          ...
          <div className="border-border-light bg-bkg-light rounded border p-4 text-center">
            <div className="flex items-center justify-center gap-1">
              <svg
                className="text-content-dark h-5 w-5"
                fill="none"
                viewBox="0 0 24 24"
                stroke="currentColor"
                aria-hidden="true"
              ...
              <span className="text-content-dark text-2xl font-bold">{member.postsCount}</span>
            </div>
        ...
        {member.showBadges !== false && (member.badgesCount ?? 0) > 0 && (
          <div className="mb-8">
            <h2 className="text-content-dark mb-4 text-lg font-semibold">Badges</h2>
        ...
        {posts.length > 0 && (
          <div>
            <h2 className="text-content-dark mb-4 text-lg font-semibold">Recent Activity</h2>
            <div className="space-y-4">
              {posts.map((post) => (
                <div key={post.id} className="border-border-light bg-bkg-light rounded border p-4">
                  ...
                  {post.title && (
                    <h3 className="text-content-dark mb-1 font-semibold">{post.title}</h3>
                  )}
                  <p className="text-content-dark-secondary line-clamp-3">{post.content}</p>
